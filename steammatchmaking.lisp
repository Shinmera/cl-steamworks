  ...
  (let ((count (compute-filters-count filters)))
    (cffi:with-foreign-objects ((array :pointer count)
      ...
      (let* ((response (or response (make-instance 'server-list-response)))
             (handle (ecase list
                       (:favorites (steam::matchmaking-servers-request-favorites-server-list (handle interface) (handle app) array count (handle response)))
                       (:friends (steam::matchmaking-servers-request-friends-server-list (handle interface) (handle app) array count (handle response)))
                       (:history (steam::matchmaking-servers-request-history-server-list (handle interface) (handle app) array count (handle response)))
